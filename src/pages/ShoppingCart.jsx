            ...
            <div className="flex flex-col lg:flex-row px-4 sm:px-8 lg:px-16 py-6 sm:py-10 lg:py-20 justify-between gap-6 lg:gap-8">
                <div className='w-full lg:w-[65%] xl:w-[850px]'>
                    ...
                    <div className="mt-6 sm:mt-8">
                        ...
                        {cartItems.length === 0 ? (
                            <p className="text-center text-gray-500 py-8 sm:py-10 text-base sm:text-lg">Your shopping cart is empty.</p>
                        ) : (
                            cartItems.map((item) => (
                                <div key={`${item.category}-${item.id}`} className="py-4 border-b">
                                    ...
                                    <div className="sm:hidden flex items-center justify-between gap-2">
                                        ...
                                        <div className="flex-1 min-w-0 px-2">
                                            <p className="font-medium text-xs leading-tight break-words">{item.name}</p>
                                        ...
                                        <div className="flex items-center border border-[#E9E9E9] px-1 py-1 rounded-lg bg-white">
                                            ...
                                            <span className="px-1 text-xs min-w-[1.5rem] text-center">{item.quantity}</span>
                                            <button
                                                onClick={() => handleQuantityChange(item, item.quantity + 1)}
                                    ...
                                    <div className="hidden sm:flex sm:justify-between sm:items-center">
                                        <div className="flex items-center space-x-3 sm:w-1/2">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="font-medium text-base leading-tight break-words">{item.name}</p>
                                            </div>
                                        </div>
                                        
                                        <p className="w-1/6 text-center text-sm md:text-base">Rs {item.price.toFixed(2)}</p>
                                        ...
                                        <div className="flex items-center w-1/6 justify-center border border-[#E9E9E9] px-3 py-2 rounded-lg bg-white">
                                            ...
                                            <span className="px-2 text-base min-w-[2rem] text-center">{item.quantity}</span>
                                            <button
                                                onClick={() => handleQuantityChange(item, item.quantity + 1)}
                                        ...
                                        <p className="font-semibold text-base w-1/6 text-center">Rs {(item.price * item.quantity).toFixed(2)}</p>
