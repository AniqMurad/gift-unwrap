...
const Giftsforbabies = () => {
  ...
  const giftsForBabiesProducts = products
    .filter(
      (product) =>
    ...
    <div>
      ...
      <div className="bg-[#FBF4E8] justify-center gap-2 sm:gap-4 lg:gap-8 flex flex-wrap text-[10px] sm:text-[12px] lg:text-[14px] font-semibold text-[#1F1F1F] uppercase py-4 sm:py-6 px-2">
        <p
          className={`cursor-pointer ${
            selectedCategory === "newborn" ? "underline" : ""
          }`}
          onClick={() => handleCategorySelect("newborn")}
        >
          <span className="hidden sm:inline">Gifts For New Born</span>
          <span className="sm:hidden">New Born</span>
        </p>
      </div>
      ...
      <div className="px-4 sm:px-8 lg:px-16 py-6 sm:py-8 lg:py-10 flex justify-center">
        ...
        <div className="w-full transition-all duration-300">
          ...
          {/* products */}
          {giftsForBabiesProducts.length > 0 ? (
            <div
              className={`justify-items-center grid gap-3 sm:gap-4 lg:gap-6 mt-6 sm:mt-8 lg:mt-10 transition-all duration-300
                ...
                `}
            >
              {giftsForBabiesProducts.slice(0, visibleCount).map((product) => (
                <Product
                  key={product.id}
                  product={{ ...product, category: "giftsForBabies" }}
                  columns={columns}
                />
              ))}
            </div>
          ) : null}
          {giftsForBabiesProducts.length > visibleCount && (
