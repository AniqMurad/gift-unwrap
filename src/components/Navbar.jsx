...
const Navbar = ({
  ...
  const { cartItems, removeFromCart, getTotalCartAmount, getTotalCartItems } =
    useCart();
  ...
  const handleMyProfile = () =>
    navigate("/myAccount", { state: { activeTab: "account" } });
  ...
  const handleContinueShoppingInDropdown = () => {
    // Renamed to avoid conflict if you have another handleContinueShopping
    setIsCartOpen(false);
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (cartRef.current && !cartRef.current.contains(event.target)) {
        ...
        const cartIcon = cartButton?.querySelector(
          'svg[aria-label="Cart Icon"]'
        );
        if (cartIcon) return;
        setIsCartOpen(false);
      }
            ...
            <DropdownMenuContent className="w-56 bg-white p-2 shadow-lg border border-grey-200">
              {isLoggedIn ? (
                ...
                <>
                  <DropdownMenuItem>
                    <Button
                      className="bg-black text-white font-bold w-full cursor-pointer"
                      onClick={handleLogin}
                    >
                      LOGIN
                    </Button>
                ...
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                  {wishlist.map((item) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => toggleWishlist(item, item.category)}
                ...
                <div className="mt-4 pt-4 border-t">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-md font-bold">
                      PKR {getTotalCartAmount()}
                    </span>
                  </div>
                  <div className="flex flex-col justify-center items-center space-y-2">
                    <button
                      className="bg-black text-white py-2 px-4 rounded-lg w-full hover:bg-gray-800 transition-colors"
                    ...
                    <button
                      ...
                      onClick={handleContinueShoppingInDropdown}
                    >
                      Continue Shopping
                    </button>
                  </div>
          ...
          <div className="absolute right-4 top-16 z-50 bg-white border rounded-md shadow-md p-4 w-64 space-y-4">
            ...
            <div className="space-y-2">
              <button
                ...
                onClick={isLoggedIn ? handleMyProfile : handleLogin}
              >
