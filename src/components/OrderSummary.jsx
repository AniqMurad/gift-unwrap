    ...
    return (
        <>
            ...
            <div className="bg-[#F7F7F7] p-5 md:p-6 rounded-xl">
                <h2 className='text-[24px] md:text-[30px] font-semibold mb-6'>{title}</h2>
                {cartItems && cartItems.length > 0 ? (
                    <div className="space-y-4 max-h-80 overflow-y-auto mb-6 pr-2"> {/* Consider if max-h is always needed */}
                        {cartItems.map(item => (
                            <div key={item.id ? `${item.category}-${item.id}` : `${item.name}-${item.price}`} className="flex items-center gap-4 border-b pb-4 last:border-b-0 last:pb-0">
                     ...
                     <p className="text-center text-gray-500 py-5">No items in this order.</p>
                )}
                ...
                <div className='space-y-3 border-t pt-6'>
                    ...
                    <div className='flex justify-between text-[16px]'>
                        <p>Shipping</p>
                        <p className='font-medium'>{shippingCost === 0 ? 'Free' : `Rs ${shippingCost?.toFixed(2) || '0.00'}`}</p>
                    </div>
                    {discountAmount > 0 && (
